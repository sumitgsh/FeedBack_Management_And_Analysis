    ...
    if ($conn->connect_error) {
        ...
        if ($type == 'rating') {
            if ($id == 'all') {
                ...
                ,teacher WHERE teacher.department_Id='$department_Id' AND teacher.teacher_Id=fr.issued_By AND question_Type='$type' and f.feedback_id=fr.feedback_R_Id and q.question_Id=f.question_Id GROUP BY q.question_Id";
            } else {
                $filter = "SELECT q.question_Id, `question`, `question_Type`,AVG(answer) as ans FROM `question`as q,`feedback_receiveables` as fr,`feedback` as f 
                ,teacher WHERE teacher.department_Id='$department_Id' AND teacher.teacher_Id=fr.issued_By AND question_Type='$type'  and f.feedback_id=$id and q.question_Id=f.question_Id GROUP BY q.question_Id";
                $detail = "SELECT session,year,department.name as dn,teacher.name as tn,course_Name FROM feedback_receiveables,coursetaught,course,department,teacher WHERE feedback_receiveables.feedback_R_Id=$id AND feedback_receiveables.issued_By=teacher.teacher_Id AND course.course_Code=coursetaught.course_Code";
                $detail_result = $conn->query($detail);
                if ($detail_result->num_rows > 0) {
                    while ($drow = $detail_result->fetch_assoc()) {
                        $session = $drow['session'];
                        $year = $drow['year'];
                        $department = $drow['dn'];
                        $teacher = $drow['tn'];
                        $course_Name = $drow['course_Name'];
                    }
                }
                $info = "Course: " . $course_Name . " | Teacher: " . $teacher . " | Department: " . $department . " | Session: " . $session . " | Year: " . $year;
            }
...
<head>
    ...
    <title>Analyse | HoD Dashboard</title>
                            ...
                            <div class="col-md-2">
                                <div class="filter_field">
                                    ...
                                    <select style="height:2.5rem;width:100%;" class="form-select form-select-lg mb-3"
                                        ...
                                        $id = "SELECT `feedback_R_Id`, `status` FROM `feedback_receiveables`,teacher WHERE teacher.department_Id='$department_Id' AND teacher.teacher_Id=feedback_receiveables.issued_By";
